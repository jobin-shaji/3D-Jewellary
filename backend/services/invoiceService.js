  ...
  static prepareInvoiceData(order, user) {
    ...
    const itemsWithTotals = order.items.map((item, index) => {
      ...
      const processedItem = {
        ...
        primaryImage: item.product?.images?.find(img => img.is_primary) || item.product?.images?.[0] || null,
        ...
        displayMetals: item.variant?.metal && item.variant.metal.length > 0 ? 
          item.variant.metal : 
          (item.product?.metals || []),
        // Display price - use variant price if available
        displayPrice: item.variant?.totalPrice || item.product?.totalPrice || item.price || 0
      };
